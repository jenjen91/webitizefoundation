Router.configure({
  ...
  waitOn: function() {
    return Meteor.subscribe('sts');
  },
  onAfterAction: function () {
    function titlet() {
     return Sts.findOne().title
    }
...
baseController = RouteController.extend({
  ...
  frontpageController = baseController.extend({
    ...
    data: {
      ...
      postsFeatured: function() {return Posts.find({post_featured: 'yes'}, {sort: {savedAt: -1}, limit: 10});
      }
    },
    waitOn: function() {
      return [
        Meteor.subscribe('posts'),
        Meteor.subscribe('sts')
      ];
    }
  });
